		...
		try:
			...
			if G in['hot','new']:
				for A in L('div.txt ul li').items():
					...
					E['list'].append({'vod_id':A('a').attr('href'),'vod_name':A('a').text(),'vod_pic':'./qyg8.png','vod_remarks':A('#actor').text(),'style':{'type':'list'}})
	...
	def searchContent(G,key,quick,page='1'):
		...
		try:
			...
			for E in J('div.txt ul li').items():
				...
				A['list'].append({'vod_id':E('a').attr('href'),'vod_name':E('a').text(),'vod_pic':'./qyg8.png','vod_remarks':E('#actor').text(),'style':{'type':'list'}})
