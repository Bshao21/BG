...
_SPADE_KEY_CACHE: Dict[str, Optional[str]] = {}

def _hg_spade_to_key(spade_a_b64: str) -> Optional[str]:
    cache_key = str(spade_a_b64 or "")
    if cache_key in _SPADE_KEY_CACHE:
        return _SPADE_KEY_CACHE[cache_key]
    try:
        raw = base64.b64decode(cache_key + "=" * (-len(cache_key) % 4), validate=False)
        if len(raw) < 16:
            _SPADE_KEY_CACHE[cache_key] = None
            return None
        ...
        try:
            ...
            return None
        result = key.lower()
        _SPADE_KEY_CACHE[cache_key] = result
        return result
    except Exception:
        _SPADE_KEY_CACHE[cache_key] = None
        return None
