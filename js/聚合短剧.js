...
async function md5(str) {
    return Crypto.MD5(str).toString(Crypto.enc.Hex).toLowerCase();
}
...
async function home(filter) {
    const classes = aggConfig.platformList.map(p => ({
        type_name: p.name,
        type_id: p.id
    }));
    const filters = {};
...
async function recommend() {
    ...
    const videos = await category(platId, 1, false, {
        area
    });
    const list = JSON.parse(videos).list.slice(0, 10).map(v => ({
        ...v,
        vod_content: `${randomPlat.name} | ${v.vod_remarks}`
    }));
    return JSON.stringify({
        list
    });
    ...
    try {
        if (tid === '七猫') {
            ...
            if (res?.data) videos.push(...res.data.map(i => ({
                ...
                vod_name: i.title,
                vod_pic: i.cover,
                vod_remarks: `更新至${i.totalChapterNum}集`
            })));
        } else if (tid === '锦鲤') {
            ...
            items.forEach(i => {
                ...
                videos.push({
                    vod_id: `番茄@${v.series_id || v.book_id || ''}`,
                    vod_name: v.title,
                    vod_pic: v.cover || v.horiz_cover,
                    vod_remarks: v.sub_title || ''
                });
            });
        } else if (tid === '星芽') {
            ...
            if (res?.data?.list) videos.push(...res.data.list.map(i => ({
                ...
                vod_name: i.theater.title,
                vod_pic: i.theater.cover_url,
                vod_remarks: `${i.theater.total}集`
            })));
        } else if (tid === '西饭') {
            ...
            if (res?.data) videos.push(...res.data.map(i => ({
                vod_id: `围观@${i.oneId}`,
                vod_name: i.title,
                vod_pic: i.vertPoster,
                vod_remarks: `集数:${i.episodeCount} 播放:${i.viewCount}`
            })));
        } else if (tid === '碎片') {
            ...
            let key = Crypto.enc.Utf8.parse("p0sfjw@k&qmewu#w");
    ...
    try {
        if (platId === '七猫') {
            ...
            if (res) vod = {
                ...
                vod_play_from: '百度短剧',
                vod_play_url: (res.data || []).map(i => `${i.title}$${i.video_id}`).join('#')
            };
        } else if (platId === '锦鲤') {
            ...
            if (res?.data) {
                ...
                vod = {
                    ...vod,
                    vod_name: b.book_name,
                    vod_pic: b.thumb_url,
                    vod_remarks: `更新至${res.data.item_data_list?.length}集`,
                    vod_play_from: '番茄短剧',
                    vod_play_url: u
                };
            }
            ...
            const headers = await getXingyaHeaders();
            const res = JSON.parse(await request(did, {
                headers
            }));
            if (res?.data) {
                const d = res.data;
            ...
            if (res?.data?.length) {
                ...
                vod = {
                    ...vod,
                    vod_name: d[0].title,
                    vod_pic: d[0].vertPoster,
                    vod_remarks: `共${d.length}集`,
                    vod_play_from: '围观短剧',
                    vod_play_url: d.map(e => `${e.title}第${e.playOrder}集$${e.playSetting}`).join('#')
                };
            }
        } else if (platId === '碎片') {
            ...
            if (tokenRes?.data?.token) {
                ...
                const res = JSON.parse(await request(`${plat.host}${plat.url2}?videoCode=${videoCode}&itemId=${itemId}`, {
                    headers
                }));
                if (res) {
                    ...
                    vod = {
                        ...
                        vod_name: d.title,
                        vod_pic: pic,
                        vod_remarks: `共${d.episodesMax || 0}集`,
                        vod_play_from: '碎片剧场',
                        vod_play_url: u
                    };
                }
...
async function play(flag, id) {
    ...
    if (/锦鲤/.test(flag)) {
        try {
            const html = await request(`${id}&auto=1`, {
                headers: {
            ...
            if (match) {
                ...
                if (data.url) return JSON.stringify({
                    parse: 0,
                    url: data.url
                });
            }
    ...
    if (/番茄/.test(flag)) {
        try {
            ...
            if (res?.data?.[id]?.video_model) {
                ...
                if (mainUrl) return JSON.stringify({
                    parse: 0,
                    url: base64Decode(mainUrl)
                });
            }
        } catch {}
    }
    if (/软鸭/.test(flag)) {
        const response = await request(`${aggConfig.platform.软鸭.host}/API/playlet/?video_id=${id}&quality=1080p`, {
            headers: aggConfig.headers.default
        });
        ...
        try {
            ...
            if (tid === '百度') {
                ...
                data = (JSON.parse(res)?.data || []).map(i => ({
                    ...
                    vod_name: i.title,
                    vod_pic: i.cover,
                    vod_remarks: `百度短剧｜更新至${i.totalChapterNum}集`
                }));
            } else if (tid === '锦鲤') {
                ...
                data = (JSON.parse(res)?.data?.list || []).map(i => ({
                    vod_id: `锦鲤@${i.vod_id}`,
                    vod_name: i.vod_name,
                    vod_pic: i.vod_pic,
                    vod_remarks: `锦鲤短剧｜${i.vod_total}集`
                }));
            } else if (tid === '番茄') {
                res = await request(`${plat.search}?keyword=${encodeURIComponent(wd)}&page=${page}`);
                data = (JSON.parse(res)?.data || []).map(i => ({
                    vod_id: `番茄@${i.series_id}`,
                    vod_name: i.title,
                    vod_pic: i.cover,
                    vod_remarks: `番茄短剧｜${i.sub_title}`
                }));
            } else if (tid === '星芽') {
                const headers = await getXingyaHeaders();
                res = await request(plat.host + plat.search, {
                ...
                data = (JSON.parse(res)?.data?.theater?.search_data || []).map(i => ({
                    vod_id: `星芽@${plat.host}${plat.url2}?theater_parent_id=${i.id}`,
                    vod_name: i.title,
                    vod_pic: i.cover_url,
                    vod_remarks: `星芽短剧｜${i.total}集 播放:${i.play_amount_str}`
                }));
            } else if (tid === '西饭') {
                const ts = Math.floor(Date.now() / 1000);
                ...
                data = (JSON.parse(res)?.data || []).map(i => ({
                    ...
                    vod_name: i.title,
                    vod_pic: i.vertPoster,
                    vod_remarks: `围观短剧｜集数:${i.episodeCount} 播放:${i.viewCount}`
                }));
            } else if (tid === '碎片') {
                let openId = (await md5(guid())).substring(0, 16);
                let body = JSON.stringify({
